...
<a href="calendrier/placerGifDistant?ID_JOUR=${jour.date}">Placer un Gif distant</a><br>
<a href="calendrier/televerserGif?ID_JOUR=${jour.date}">Téléverser un Gif</a>
</td>
</c:when>
<c:when test="${jour.gif ne null }">
<td>
<c:if test="${jour.getGif().getLegende() ne null && jour.gif.legende ne ''}"><h2>${jour.gif.legende}</h2></c:if>
<c:if test="${jour.gif.getClass().simpleName eq 'GifDistant'}"><img src="${jour.gif.url}" height="200" alt="Super image du calendrier !"></c:if>
<c:if test="${jour.gif.getClass().simpleName eq 'GifTeleverse'}"><img src="images/${jour.gif.nomFichierOriginal}" height="200" alt="Super image du calendrier !"></c:if>
</td>
